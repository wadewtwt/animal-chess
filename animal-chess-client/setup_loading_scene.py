...
def get_base64_uuid(u):
    # Cocos encodes UUIDs in base64 sometimes, but for scene files, it usually uses the raw UUID.
    return u

def create_loading_scene():
    # Generate UUID for LoadingScene script
    ...
    with open('assets/scripts/LoadingScene.ts.meta', 'w') as f:
    ...
    # Generate meta for loading.scene
    scene_meta = {
      "ver": "1.0.12",
      "importer": "scene",
      "imported": True,
    ...
    print(f"Created loading.scene with UUID {scene_asset_uuid}")
...
if __name__ == '__main__':
    scene_uuid = create_loading_scene()
